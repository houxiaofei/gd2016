...
void initALL(void)
{
	...
	initModesAndClock();        //时钟模块，系统时钟设置为锁相环，超频到80M  
	enableIrq();                //中断初始化
	initEMIOS_0MotorAndSteer();   //电机舵机的EMIOS模块初始化（改）
	initEMIOS_0ModulusCounter();  //EMIOS的脉冲捕捉功能初始化（改）
//	initEMIOS_0Image();
	initLINFlex_0_UART();         //UART，蓝牙
	initSTM();                    //计时器
	initAD();                     //ADC  （改）
...
/*************************光编初始化***********************/
void initEMIOS_0ModulusCounter(void) //D12,A8模数计数器入口，上升沿，C9,C15光编正反转
{
	//D12
	...
	SIU.PCR[41].R = 0x0102;  //PD11左轮光编正反转 ，高电平正转，低电平反转
	...
	SIU.PCR[47].R=0x0102;  //A6右轮光编正反转，高电平反转，低电平正转
}
...
void initAD(void)
{
  ...
  ADC.NCMR[1].R = 0x00000007;   /*使能CH32,CH33和CH34通道（标准通道）的位正常采样*/
